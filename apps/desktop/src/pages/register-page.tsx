...
export function RegisterPage() {
  const { registerByPassword } = useAuth();
  ...
  return (
    <div className={styles.shell}>
      <div className={styles.card}>
        ...
        <p className={styles.subtitle}>注册本地账号，数据保存在本机</p>
        <input
          className="wx-input"
          value={username}
        ...
        <button
          ...
          onClick={async () => {
            ...
            setLoading(true);
            setHint('');
            const err = await registerByPassword(username.trim(), password);
            setLoading(false);
            if (err) setHint(err);
          }}
          disabled={loading}
        ...
        <div className={styles.authLinkRow}>
          <button type="button" className={styles.authLink} onClick={() => navigate('/auth/login')} disabled={loading}>
            已有账号？去登录
          </button>
        </div>
      </div>
